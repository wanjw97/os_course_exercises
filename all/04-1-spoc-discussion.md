...
覆盖技术是划分程序功能模块，不会同时执行的模块共享一块内存
程序员需要划分模块

2. 什么是交换技术？覆盖与交换有什么不同？
交换技术是作为一个整体的进程地址空间的换入与换出
...
时间局部性：某一时刻访问某一内存，很可能在不久后再次访问。
空间局部性：访问某一块内存后，不久后很可能访问附近的存储内容。
分支局部性：
